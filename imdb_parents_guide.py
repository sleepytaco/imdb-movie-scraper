...
def main(movie=''):
    ...
    while movie != 'q':
        ...
        print(f"Gathering info for '{movie}'...")
        ...
        soup = BeautifulSoup(site.text, 'lxml')
        try:
            movie_name_and_year = ' '.join(
                soup.find_all('div', class_='title_wrapper')[0].find_all('h1')[0].text.split('\xa0'))
        except IndexError:
            ##### SPACER #####
            for i in range(30):
                print()
        ...
        try:
            nudity_level = \
            soup.find_all('section', {'id': 'advisory-nudity'})[0].find_all('li')[0].text.strip().split('\n')[0]
        except IndexError:
            nudity_level = "N/A"
